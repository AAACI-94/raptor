...
Respond with the JSON structure specified in your system prompt."""
        ...
        # Add citation provenance warning: sources are LLM-generated, not API-verified
        sources = payload.get("sources", [])
        if sources:
            payload["citation_warning"] = (
                f"IMPORTANT: {len(sources)} sources were generated by the LLM, not retrieved from "
                "verified APIs (Semantic Scholar, arXiv, etc.). URLs and specific claims may be "
                "hallucinated. Authors MUST verify each citation before submission. "
                "v1.1 will add API-backed source retrieval."
            )
            for source in sources:
                source["verification_status"] = "unverified_llm_generated"
